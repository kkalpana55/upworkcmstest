...
namespace upworkcmstest.tests
{
    [TestFixture]
    [Parallelizable]
    internal class Rubytestcase:Basetest
    {
        ...
        public void Tc9(string browsername)
        {
            ...
            test = extent.CreateTest("<h3>Ruby's page</h3> ").Info("<h3>Test Started in </h3>" + browsername + " <h3>browser ---->Verifying Ruby's Page</h3>");
            parallelbrowser(browsername);
            var lp = new Rubypage(driver);
            lp.verifyingrubypage();
